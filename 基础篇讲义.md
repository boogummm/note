...
3. 空间复杂度：O(n+k)，其中k为桶的数量。一般来说n>>k，因此额外空间需要大概n个左右
...
4. add(Object o) 首次扩容为 10，**再次扩容为上次容量的 1.5 倍**（具体过程是len+len>>1，如15，扩容后是22）
...
* 没有采用这一设计的典型例子是 Hashtable，其每次扩容为原来的二倍加一（如0,11,23,47,95）
